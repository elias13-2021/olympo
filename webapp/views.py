...
@login_required
def detalleActividad(request, codactividad):
    #cliente = Registros_cliente.objects.get(pk=DNI)
    actividad = get_object_or_404(Registros_actividad, pk=codactividad)
    return render(request, 'crudActividad/detalleActividad.html', {'actividad': actividad})

@login_required
def editarActividad(request, codactividad):
    actividad = get_object_or_404(Registros_actividad, pk=codactividad)
    if request.method == 'POST':
    ...
    return render(request, 'crudActividad/editarActividad.html', {'formaActividad': formaActividad})
...
def detalleRegistro(request, codregistro):
    #cliente = Registros_cliente.objects.get(pk=DNI)
    registro = get_object_or_404(Registros_registro, pk=codregistro)
    return render(request, 'crudRegistros/detalleRegistro.html', {'registro': registro})
...
@login_required
def listadoRegistroTM2(request):
    listadotm2 = Registros_registro.objects.filter(codactividad__horasalida__contains='09:00')

    return render(request, 'listados/listadoRegistroTM2.html', {'listadotm2': listadotm2})